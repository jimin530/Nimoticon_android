...
public class TurboImageView extends View implements MultiTouchObjectCanvas<MultiTouchObject> {
    ...
    public void addSetObject(Context context, Bitmap bitmap, float x, float y) {
        ImageObject imageObject = new ImageObject(bitmap, context.getResources());
        addSetObject(context, imageObject, x, y);
    }

    private void addSetObject(Context context, ImageObject imageObject, float x, float y) {
        deselectAll();
        ...
        float cx = getX() + getWidth() / 2;
        float cy = getY() + getHeight() / 2;
        mImages.get(mImages.size() - 1).init(context, x, y);
